...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [subBtn setTitleColor:UIColorFromHex(0xfa5d7c, 1) forState:UIControlStateNormal];
        ...
        [cancleBtn setImage:[UIImage imageNamed:@"cancleList_woman"] forState:UIControlStateNormal];
...
- (void)officeBtnClick:(UIButton *)sender{
    if (sender.tag == 0) {
        ...
        sender.backgroundColor = UIColorFromHex(0xfa5d7c, 1);
...
- (void)superSelectBtnAction:(NSInteger) tag{
    for (NSString *key in _btnDic.allKeys){
        if (key.integerValue == tag) {
            ...
            button.backgroundColor = UIColorFromHex(0xfa5d7c, 1);
