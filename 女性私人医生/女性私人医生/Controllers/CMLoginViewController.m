...
- (void)viewDidLoad {
    ...
    UIImageView *logoImageView = [[UIImageView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-70*SCREEN_WIDTH/375)/2 ,
                                                                               ...
                                                                               70*SCREEN_WIDTH/375,
                                                                               70*SCREEN_WIDTH/375)];
    logoImageView.image = [UIImage imageNamed:@"woman"];
    ...
    [loginWithTel setTitleColor:UIColorFromHex(0xf65378, 1) forState:UIControlStateNormal];
    ...
    underLine = [[UILabel alloc] initWithFrame:CGRectMake(188*SCREEN_WIDTH/375, 202*SCREEN_HEIGHT/667, 145*SCREEN_WIDTH/375, 3)];
    [underLine setBackgroundColor:UIColorFromHex(0xf65378, 1)];
    ...
    [loginBtn setBackgroundColor:UIColorFromHex(0xf65378, 1)];
    ...
    [underLine bringSubviewToFront:self.view];
...
- (void)loginWithAccountBtnClick{
    ...
    [loginWithAccount setTitleColor:UIColorFromHex(0xf65378, 1) forState:UIControlStateNormal];
...
- (void)recieveAuthResponse:(BOOL)isSucced code:(NSString *)code{
    ...
    NSString *post = [NSString stringWithFormat:@"source=apple&imei=%@&addrdetail=%@&ersion=%@&appid=1&deviceid=%@&os=ios",[CureMeUtils defaultCureMeUtil].UDID,[CureMeUtils defaultCureMeUtil].encodedLocateInfo,[CureMeUtils defaultCureMeUtil].appVersion,[CureMeUtils defaultCureMeUtil].uniID];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (userID) {
                ...
                NSString *urlStr = [NSString stringWithFormat:@"https://api.weixin.qq.com/sns/oauth2/access_token?appid=%@&secret=%@&code=%@&grant_type=authorization_code",WX_WOMAN_ID,WX_WOMAN_SECRET,backCode];
